...
function startGame() {
  ...
  document.getElementById("status").innerHTML = "Game Status: Loading";
}

function game() {
  //console.log(noseX+" test "+noseY);
  instializeInDraw();
  ...
  if (gameConfig.status === 'gameover') {
    ...
    textSize(15);
    text("click the Restart Button to restart", gameConfig.screenX / 2, gameConfig.screenY / 2 + 135);
    textSize(40);
...
function changeGameStatud(character) {
  ...
  if (gameConfig.status === "gameover" && Restart) {
